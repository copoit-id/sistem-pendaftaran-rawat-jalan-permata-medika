            ...
            <ul class="space-y-2 font-medium">
                <li>
                    <a href="{{ url('/admin/dashboard') }}"
                        class="flex items-center p-3 {{ $title == 'Dashboard' ? 'bg-red2 text-red' : 'text-gray-900 hover:bg-gray-100' }} rounded-lg dark:text-white dark:hover:bg-gray-700 group">
                        <svg class="flex-shrink-0 w-5 h-5 {{ $title == 'Dashboard' ? ' text-red' : 'text-gray-500  group-hover:text-gray-900' }} transition duration-75 dark:text-gray-400 dark:group-hover:text-white"
                ...
                <li>
                    <button type="button"
                        class="flex items-center w-full p-3 text-base {{ $title == 'Pesanan' ? 'bg-red2 text-red' : 'text-gray-900 hover:bg-gray-100' }} transition duration-75 rounded-lg group dark:text-white dark:hover:bg-gray-700"
                        aria-controls="dropdown-example" data-collapse-toggle="dropdown-example">
                        <svg class="flex-shrink-0 w-5 h-5 {{ $title == 'Pesanan' ? ' text-red' : 'text-gray-500  group-hover:text-gray-900' }} transition duration-75  dark:text-gray-400 dark:group-hover:text-white"
                            aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                            viewBox="0 0 18 21">
                            <path
                                d="M15 12a1 1 0 0 0 .962-.726l2-7A1 1 0 0 0 17 3H3.77L3.175.745A1 1 0 0 0 2.208 0H1a1 1 0 0 0 0 2h.438l.6 2.255v.019l2 7 .746 2.986A3 3 0 1 0 9 17a2.966 2.966 0 0 0-.184-1h2.368c-.118.32-.18.659-.184 1a3 3 0 1 0 3-3H6.78l-.5-2H15Z" />
                        </svg>
                        <span class="flex-1 ms-3 text-left rtl:text-right whitespace-nowrap">Pesanan</span>
                        ...
                        <span
                            class="inline-flex items-center justify-center w-3 h-3 p-3 ms-3 text-sm font-medium {{ $title == 'Pesanan' ? 'bg-red text-white' : 'text-blue-800 bg-blue-100' }} rounded-full dark:bg-blue-900 dark:text-blue-300">3</span>
                    </button>
                    <ul id="dropdown-example" class="hidden py-2 space-y-2">
                        <li>
                            <a href="{{ url('/admin/pesanan') }}"
                                class="flex items-center w-full p-3 text-gray-500 transition duration-75 rounded-lg pl-11 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700">Pesanan
                                Masuk</a>
                        </li>
                        <li>
                            <a href="{{ url('/admin/pesanan') }}"
                ...
                <li>
                    <a href="{{ url('/admin/karyawan') }}"
                        class="flex items-center p-3 {{ $title == 'Karyawan' ? 'bg-red2 text-red' : 'text-gray-900 hover:bg-gray-100' }}  rounded-lg dark:text-white dark:hover:bg-gray-700 group">
                        <svg class="flex-shrink-0 w-5 h-5 {{ $title == 'Karyawan' ? ' text-red' : 'text-gray-500  group-hover:text-gray-900' }} transition duration-75 dark:text-gray-400 dark:group-hover:text-white"
                            aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                            viewBox="0 0 20 18">
                            <path
